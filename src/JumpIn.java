...
public class JumpIn {
	...
	private final Point[] HOLES = {new Point(0,0),new Point(2,2), new Point(0,4),new Point(4,0), new Point(4,4)}; 
	...
	private boolean checkWin() {
		...
		for(int i = 0;i<listeners.size();i++) {
			if(listeners.get(i).getClass().getName()=="Rabbit") {
	...
	private boolean processCommand(Move move) {
		if(move.getChosenAnimal().getName()=="Rabbit") {
			JumpInEvent event = new JumpInEvent(this,move.getChosenAnimal(),move.getFinalLocation(),HOLES);
		}else{
			JumpInEvent event = new JumpInEvent(this,move.getChosenAnimal(),move.getFinalLocation(),HOLES);		//fix for fox
		}
